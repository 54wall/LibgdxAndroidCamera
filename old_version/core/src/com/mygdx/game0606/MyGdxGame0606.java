...
public class MyGdxGame0606 implements ApplicationListener {
	...
	private List<Texture> texturelisg=new ArrayList<Texture>(); 
...
//		faceTexture_4=new Texture(Gdx.files.external("a_SDK_fail/4.jpg"));
		...
		camera = new PerspectiveCamera(67.0f, 2.0f * Gdx.graphics.getWidth()
				/ Gdx.graphics.getHeight(), 2.0f);// oldfloat// fieldOfViewY改为37无反应
	...
	public void render() {
		...
		faceTexture=new Texture(Gdx.files.external("a_SDK_fail/"+i_render+".jpg"));
		batch.begin();		
...
//		render_preview();
	 ...
	 * 2现在实现的功能是按下左按钮，开始预览，按下右侧按钮进行聚焦拍摄，这说明在mode为preview状态时，button是可以相应的之前却一直不可以*/
